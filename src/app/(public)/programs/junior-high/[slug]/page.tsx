...
import { JHS_SUBJECTS, getJhsSubjectBySlug } from "../../jhs-subjects";

export function generateStaticParams() {
  return JHS_SUBJECTS.map((subject) => ({ slug: subject.slug }));
}

export default async function JhsSubjectPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const subject = getJhsSubjectBySlug(slug);
      ...
      <section className="bg-white py-12 sm:py-16">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6">
          ...
          <div className="mt-6 rounded-xl border border-blue-100 bg-blue-50/50 p-5 sm:p-6">
            <h2 className="text-lg font-bold text-gray-900">Enroll in Junior High</h2>
            <p className="mt-2 text-sm leading-relaxed text-gray-600 sm:text-base">
              {subject.name} is part of our Junior High School program for Grades 7–10. Visit admissions
              to learn about requirements, enrollment steps, and tuition information.
            </p>
            <Link
              href="/admissions"
