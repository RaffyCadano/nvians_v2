...
import { getAllTrackSubjectParams, getTrackSubject } from "../../../../shs-tracks";

export function generateStaticParams() {
  return getAllTrackSubjectParams().map(({ trackSlug, subjectSlug }) => ({
    trackSlug,
    subjectSlug,
  }));
}

export default async function ShsTrackSubjectPage({
  params,
}: {
  params: Promise<{ trackSlug: string; subjectSlug: string }>;
}) {
  const { trackSlug, subjectSlug } = await params;
  const result = getTrackSubject(trackSlug, subjectSlug);

  if (!result) {
    notFound();
  }

  const { track, subject } = result;
    ...
    <div>
      <section className="relative flex min-h-[45vh] items-center overflow-hidden text-white sm:min-h-[42vh] lg:min-h-[40vh]">
        ...
        <div className="container relative z-10 mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:py-16">
          <p className="mb-2 text-xs font-medium tracking-wider text-yellow-400 uppercase sm:mb-3 sm:text-sm lg:text-base">
            Senior High · {track.name} Track · Grades 11–12
          </p>
      ...
      <section className="bg-white py-12 sm:py-16">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6">
          <div className="grid gap-8 lg:grid-cols-2 lg:items-center">
            <div>
              ...
              <p className="mt-4 text-sm text-gray-500">
                Part of the <span className={`font-semibold ${track.color}`}>{track.name}</span> track —{" "}
                {track.fullName}.
              </p>
            </div>
          ...
          <div className={`mt-6 rounded-xl border p-5 sm:p-6 ${track.bg}`}>
            <h2 className="text-lg font-bold text-gray-900">Other {track.name} Subjects</h2>
            <ul className="mt-4 space-y-2">
              {track.subjects
                ...
                .map((item) => (
                  <li key={item.slug}>
                    <Link
                      href={`/programs/senior-high/tracks/${track.slug}/${item.slug}`}
                      className={`inline-flex items-center gap-1 text-sm font-medium ${track.color} hover:underline`}
                    >
                      {item.name}
          ...
          <div className="mt-6 rounded-xl border border-blue-100 bg-blue-50/50 p-5 sm:p-6">
            ...
            <p className="mt-2 text-sm leading-relaxed text-gray-600 sm:text-base">
              {subject.name} is offered under the {track.name} track for Grades 11–12. Visit admissions
              to learn about track selection, requirements, and enrollment steps.
            </p>
            <Link
              href="/admissions"
